...
class Config:
    ...
    # 大盘复盘市场区域：tw(台股)、cn(A股)、hk(港股)、us(美股)、jp(日股)、kr(韩股)、both(全部市场)
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        supported_regions = ('cn', 'tw', 'hk', 'us', 'jp', 'kr', 'both')
        ordered_regions = ('cn', 'tw', 'hk', 'us', 'jp', 'kr')
        ...
        logging.getLogger(__name__).warning(
            f"MARKET_REVIEW_REGION 配置值 '{value}' 无效，已回退为默认值 'cn'（合法值：tw / cn / hk / us / jp / kr / both；支持逗号分隔有效值）"
        )
        return 'cn'
