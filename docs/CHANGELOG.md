...
## [Unreleased]
- [修復] JEAC 的 Fugle 預設優先序改為 8、FinMind 維持 1；公開 SearXNG 預設停用，避免例行報告被不穩定公開來源拖慢。
- [改進] JEAC GitHub 日報、週報、月報私有交付統一只產生 DOCX；日報固定工作日 08:00、週報固定週六 11:00（Asia/Taipei）。
...
- [修復] 台股名稱查詢命中本地索引或即時行情失敗後，不再回退到 Pytdx／Baostock 等中國市場資料源。
...
- [新功能] 新增五個可攜式 JEAC Skills，涵蓋資料品質、SEPA 波段、新聞情報、回測驗證與組合風險。
- [新功能] 新增無偷看未來資料的 SEPA Pivot 回測驗證器：訊號後隔日進場、納入成本與滑價、同日停損/目標採保守停損優先，並輸出完整績效摘要。
- [新功能] 本地日報儲存時同步產生同名 DOCX，保留 Markdown 作為通知與機器可讀來源。
